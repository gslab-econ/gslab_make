...
class ProgramDirective(Directive):
    ...
    def parse_options(self):
        option_list = self.option.replace('=', '').split()
        self.option_dict = {}

        for o in option_list:
            if o[0] in metadata.option_start_chars:
                option = o
                self.option_dict[option] = ''
            else:
                self.option_dict[option] += o + ' '

    def option_overlap_error_check(self, **kwargs):
        application = invert_dict(metadata.extensions)[self.program_ext]
        option_overlaps = metadata.option_overlaps.get(application)

        if not option_overlaps: 
            return

        for (k, v) in option_overlaps.iteritems():
            if kwargs.has_key(k) and self.option_dict.has_key(v):
                raise CritError(messages.crit_error_option_overlap % (k, v))
