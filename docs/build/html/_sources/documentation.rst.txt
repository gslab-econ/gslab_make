Specifying paths
================
The majority of the functions in :mod:`gslab_make` contain a ``paths`` argument that requires passing in a dictionary specifying default paths used for writing and logging purposes. To fully use any function in the library, the dictionary *must* contain values for the following keys (i.e., default paths):
